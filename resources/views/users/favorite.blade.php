    ...
    <div class="row">
      @foreach ($favorite_products as $favorite_product)
        <div class="col-md-7 mt-2">
          <div class="d-inline-flex">
            <a href="{{ route('products.show', $favorite_product->id) }}" class="w-25">
              ...
              <img src="{{ asset($favorite_product_image) }}" class="img-fluid w-100">
              @else
        ...
        <div class="col-md-2 d-flex align-items-center justify-content-end">
          <a href="{{ route('favorites.destroy', $favorite_product->id) }}" class="samuraimart-favorite-item-delete" onclick="event.preventDefault(); document.getElementById('favorites-destroy-form').submit();">
              削除
          </a>
        ...
        <div class="col-md-3 d-flex align-items-center justify-content-end">
          <form method="POST" action="{{ route('carts.store') }}" class="m-3 align-items-end">
            @csrf
            <input type="hedden" name="id" value="{{ $favorite_product->id }}">
            <input type="hedden" name="name" value="{{ $favorite_product->name }}">
            <input type="hedden" name="price" value="{{ $favorite_product->price }}">
            <input type="hedden" name="image" value="{{ $favorite_product->image }}">
            <input type="hidden" name="carriage" value="{{ $favorite_product->carriage_flag}}">
            <input type="hedden" name="qty" value="1">
            <input type="hedden" name="weight" value="0">
